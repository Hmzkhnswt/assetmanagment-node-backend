...
const entryItemSchema = z
  ...
  .superRefine((val, ctx) => {
    if (!val.fromAccountId && !val.sourceAccountId) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'fromAccountId or sourceAccountId is required.',
        path: ['fromAccountId'],
      });
    }
    if (!val.reasonAccountId && !val.expenseAccountId) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'reasonAccountId or expenseAccountId is required.',
        path: ['reasonAccountId'],
      });
    }
  });

const createJournalInvoiceBodySchema = z.object({
  date: optionalDateSchema,
  description: z.string().trim().min(1),
  entries: z.array(entryItemSchema).min(1),
...
const journalInvoiceQuerySchema = z.object({
  userId: z.preprocess(
    (value) => {
...
const updateJournalInvoiceBodySchema = z.object({
...
const updateInvoiceBodySchema = z
  ...
  .refine(
    (val) =>
      val.invoiceNumber !== undefined ||
      val.clientName !== undefined ||
      val.issueDate !== undefined ||
...
async function createJournalInvoice(req, res, next) {
  const body = createJournalInvoiceBodySchema.parse(req.body || {});
  ...
  try {
    ...
    res.apiSuccess('Journal invoice created successfully.', {
      invoice: {
...
async function listJournalInvoices(req, res, next) {
  try {
    const { userId } = journalInvoiceQuerySchema.parse(req.query);
    ...
    const params = hasUser ? [pattern, userId] : [pattern];
    const { rows } = await pool.query(sql, params);

    res.apiSuccess('Journal invoices retrieved successfully.', { invoices: rows });
  } catch (err) {
    next(err);
  }
}
...
async function getJournalInvoice(req, res, next) {
  try {
    const { journalEntryId } = journalEntryIdParamSchema.parse(req.params);
    ...
    res.apiSuccess('Journal invoice retrieved successfully.', {
      invoice: jeRows[0],
      lines,
    });
  } catch (err) {
...
async function updateJournalInvoice(req, res, next) {
  const body = updateJournalInvoiceBodySchema.parse(req.body || {});
  const { journalEntryId } = journalEntryIdParamSchema.parse(req.params);
  const { date, description, entries } = body;
  ...
  try {
    ...
    res.apiSuccess('Journal invoice updated successfully.', {
      invoice: {
  ...
  try {
    ...
    if (status !== undefined) {
      ...
      if (!INVOICE_STATUSES.has(normalizedStatus)) {
        throw httpError(400, `status must be one of: ${Array.from(INVOICE_STATUSES).join(', ')}.`);
      }
    }
    ...
    res.apiSuccess('Invoice header updated successfully.', { invoice: rows[0] });
  } catch (err) {
    if (err.code === '23505') {
